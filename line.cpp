...
void ReadData()
{
    ...
    /*store the edges in three arrays.*/
	...
	{
		fscanf(fin, "%lf %s %s", &weight, name_v1, name_v2);
        ...
        */
		...
		vertex[vid].degree += weight;/*why weight is added to weight*/
...
/* Sample negative vertex samples according to the power of vertex degrees */
void InitNegTable()
{
...
void TrainLINE() {
	...
	InitHashTable();/*hashtable is an array filled with -1.*/
	ReadData();/*read weighted graph in edgelist file, store in three arrays*/
	InitAliasTable();/*O(n) time and space to preprocess the probability of each edge being chosen, the sampling time is O(1).*/
	InitVector(); /*init the embedding vector and the context.*/
	InitNegTable(); /*what ?*/
	InitSigmoidTable();
